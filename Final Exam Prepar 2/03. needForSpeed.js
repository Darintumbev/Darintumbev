function needForSpeed(input) {
    let carsFound = Number(input.shift());
    let result = new Map();
    ...
    for (const element of input) {
        ...
        if (element.includes('|')) {
            let [car, mileges, fuel] = element.split('|');
            result.set(car,[]);
            result.get(car).push(Number(mileges));
            result.get(car).push(Number(fuel));
        }
        if (element.includes(' : ')){
            let token = element.split(" : ");
            if(element.includes("Refuel")){
                let givenCar = token[1];
                let givenFuel = Number(token[2]);
                ...
                let givenCar = token[1];
                let givenMileges = Number(token[2]);

                result.get(givenCar)[0] -= givenMileges;

                if(result.get(givenCar)[0] < 10000){
                    result.get(givenCar)[0] = 10000;
                }else{
                    console.log(`${givenCar} mileage decreased by ${givenMileges} kilometers`);
                }
